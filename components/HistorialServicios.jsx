      ...
      <div className="rounded-2xl border border-slate-700 bg-slate-800/90 p-5 shadow-xl shadow-slate-950/20 sm:p-6">
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h2 className="text-2xl font-bold text-white">Historial de Consumos</h2>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="cursor-pointer rounded-lg border border-slate-700 bg-slate-900 p-2.5 text-sm text-white outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-500/25"
            >
        ...
        {registrosFiltrados.length === 0 ? (
          ...
          <div className="space-y-4">
            {registrosFiltrados.map((reg) => (
              <div key={reg.id} className="flex flex-col gap-4 rounded-2xl border border-slate-700 bg-slate-900/60 p-4 transition hover:border-slate-500 md:flex-row md:items-center md:justify-between">
                <div>
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-slate-400">
                    <span>⚡ Luz: S/ {reg.luz_total?.toFixed(2)}</span>
                    <span>💧 Agua: S/ {reg.agua_total?.toFixed(2)}</span>
                    <span>🔥 Gas: S/ {reg.gas_total?.toFixed(2)}</span>
                  </div>
                </div>

                <div className="flex w-full flex-col gap-3 md:w-auto md:flex-row md:items-center">
                  <div className="rounded-xl border border-slate-700 bg-slate-800/80 px-4 py-2 text-left md:text-right">
                    <span className="block text-[10px] uppercase tracking-[0.18em] text-slate-400">Total Global</span>
                    <span className="text-xl font-black text-emerald-400">S/ {reg.monto_total_global?.toFixed(2)}</span>
                  </div>
                  <div className="flex gap-2">
                    <button
                      onClick={() => descargarReciboPDF(reg)}
                      className="rounded-lg bg-red-600 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-red-900/20 transition hover:bg-red-500"
                    >
                      📄 PDF
                    </button>
                    <button
                      onClick={() => eliminarRegistro(reg.id, reg.mes_anio, reg.casa)}
                      className="rounded-lg border border-red-500/40 bg-red-500/10 px-4 py-2 text-sm font-bold text-red-300 transition hover:bg-red-500 hover:text-white"
                    >
                      🗑 Eliminar
                    </button>
                  </div>
